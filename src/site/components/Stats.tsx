      ...
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-6">
        {stats.map(({ icon: Icon, value, label, description }, i) => (
          <Reveal key={label} delay={i * 80}>
            <div className="flex h-full flex-col gap-4 rounded-2xl border border-slate-100 bg-slate-50/60 p-5 transition-colors hover:border-teal-500/30 hover:bg-teal-50/40 sm:p-6">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-500/10 text-teal-600">
                <Icon className="h-5 w-5" strokeWidth={1.75} />
              </span>
              <div>
                <p className="font-display text-2xl font-bold tracking-tight text-navy-900 sm:text-3xl">
                  {value}
                </p>
